...
const fn glyph_5x7(character: u8) -> [u8; 7] {
    match character {
        ...
        // Lowercase letters sit within the same 7-row cell as uppercase, with
        // no true descender row available (the 8th cell row is reserved for
        // inter-line spacing). g/j/p/q/y curl their tails back up instead of
        // dropping below the baseline, matching period-correct 8-bit consoles
        // whose lowercase charsets made the same tradeoff.
        b'a' => [0x00, 0x00, 0x0e, 0x01, 0x0f, 0x11, 0x0f],
        b'b' => [0x10, 0x10, 0x10, 0x1e, 0x11, 0x11, 0x1e],
        b'c' => [0x00, 0x00, 0x0f, 0x10, 0x10, 0x10, 0x0f],
        b'd' => [0x01, 0x01, 0x01, 0x0f, 0x11, 0x11, 0x0f],
        b'e' => [0x00, 0x00, 0x0e, 0x11, 0x1f, 0x10, 0x0f],
        b'f' => [0x06, 0x09, 0x08, 0x1c, 0x08, 0x08, 0x08],
        b'g' => [0x00, 0x00, 0x0f, 0x11, 0x0f, 0x01, 0x0e],
        b'h' => [0x10, 0x10, 0x10, 0x1e, 0x11, 0x11, 0x11],
        ...
        b'q' => [0x00, 0x00, 0x0f, 0x11, 0x11, 0x0f, 0x01],
        b'r' => [0x00, 0x00, 0x16, 0x19, 0x10, 0x10, 0x10],
        b's' => [0x00, 0x00, 0x0f, 0x10, 0x0e, 0x01, 0x1e],
        b't' => [0x04, 0x04, 0x0e, 0x04, 0x04, 0x04, 0x03],
...
mod tests {
    ...
    #[test]
    fn lowercase_letters_have_distinct_bold_glyphs_within_the_cell() {
        for character in b'a'..=b'z' {
            let glyph = CHARACTER_ROM[character as usize];
            assert_ne!(glyph, [0; 8], "{} has no glyph", character as char);
            // Emboldening (the same treatment uppercase gets) never sets the
            // outer two columns, so mixed-case lines keep their cursor margins.
            for row in glyph {
                assert_eq!(row & 0x81, 0, "{} touches an outer cell edge", character as char);
            }
        }
        // Case must actually be distinguishable, not just "some glyph exists".
        assert_ne!(CHARACTER_ROM[b'a' as usize], CHARACTER_ROM[b'A' as usize]);
        assert_ne!(CHARACTER_ROM[b'o' as usize], CHARACTER_ROM[b'0' as usize]);
    }
